 ...
 */
class ASU_BASE Logger : public QObject
{
Q_OBJECT
public:
    enum MsgType
...
//
#if defined(QT_DEBUG)
    #define zLog(__msg, __type) Z::Logger::instance()->message(__msg, __type, QMessageLogContext(__FILE__, __LINE__, __FUNCTION__, zStr(Z::Logger::MsgTypeToString(__type))))
#else
    #define zLog(__msg, __type) ""
#endif
//
#define zTrace(__msg) zLog(__msg, Z::Logger::TraceMsg)
#define zDebug(__msg) zLog(__msg, Z::Logger::DebugMsg)
#define zInfo(__msg) zLog(__msg, Z::Logger::InfoMsg)
#define zNotice(__msg) zLog(__msg, Z::Logger::NoticeMsg)
#define zWarning(__msg) zLog(__msg, Z::Logger::WarningMsg)
#define zError(__msg) zLog(__msg, Z::Logger::ErrorMsg)
#define zCritical(__msg) zLog(__msg, Z::Logger::CriticalMsg)
//
#define zGroup(__name) zLog(tr("GROUP BEGIN %1").arg(__name), Z::Logger::TraceMsg);
#define zGroupEnd(__name) zLog(tr("GROUP END %1").arg(__name), Z::Logger::TraceMsg);
//
#endif // LOGGER_H
